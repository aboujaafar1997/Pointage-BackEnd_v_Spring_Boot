...
>
  ...
  recherche2=(e)=>{ 
    ...
    download=async()=>{
     ...
     let response = await fetch('http://localhost:8080/api/Pointage/list',options);
...
token:sessionStorage.getItem('token')
    ...
    ajouter=async (e)=>{
     ...
     await fetch("http://localhost:8080/api/Pointage/ajouter", {
        method: 'POST',
        headers: new Headers({
...
supprimer=async(id1,id2,date)=>{
 await fetch("http://localhost:8080/api/Pointage/supprimer/"+id1+"/"+id2+"/"+date, {
    method: 'POST',
    headers: new Headers({
        'Content-Type': 'application/json','Authorization': 'bearer '+sessionStorage.getItem('token')
    })
