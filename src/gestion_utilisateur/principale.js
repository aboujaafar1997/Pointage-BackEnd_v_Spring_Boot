...
>
  ...
  recherche=(e)=>{ 
    ...
    download=async()=>{
     ...
     let response = await fetch('http://localhost:8080/api/Utilisateur/list',options);
     ...
     let response2 = await fetch('http://localhost:8080/api/profil/list',options2);
...
if(this.state.mdp===this.state.vmdp){
      fetch("http://localhost:8080/api/Utilisateur/ajouter", {
        method: 'POST',
        headers: new Headers({
...
supprimer=async(id)=>{
  fetch("http://localhost:8080/api/Utilisateur/supprimer/"+id, {
    method: 'POST',
    headers: new Headers({
        'Content-Type': 'application/json','Authorization': 'bearer '+sessionStorage.getItem('token')
    })
