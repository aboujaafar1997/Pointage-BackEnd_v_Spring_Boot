...
>
  ...
  recherche=(e)=>{ 
    ...
    download=async()=>{
     ...
     let response = await fetch('http://localhost:8080/api/profil/list',options);
     ...
     let response2 = await fetch('http://localhost:8080/api/Privs/list',options2);
...
if(this.state.privs.length >= 1){
      fetch("http://localhost:8080/api/profil/ajouter", {
        method: 'POST',
        headers: new Headers({
...
supprimer=async(id)=>{
  fetch("http://localhost:8080/api/profil/supprimer/"+id, {
    method: 'POST',
    headers: new Headers({
        'Content-Type': 'application/json','Authorization': 'bearer '+sessionStorage.getItem('token')
    })
...
privsaffiche= async (id)=>{
  let options = {method:"POST",headers : { "charset":"utf-8",'Content-Type':'application/json','Authorization': 'bearer '+sessionStorage.getItem('token') }};
  let response = await fetch("http://localhost:8080/api/profil/get/"+id,options);
  let resultat = await response.json();
  this.setState({
    profilpris:resultat,
    idshowprofil:id,
...
show= async(id)=>{
  let options = {method:"POST",headers : { "charset":"utf-8",'Content-Type':'application/json','Authorization': 'bearer '+sessionStorage.getItem('token') }};
  let response = await fetch('http://localhost:8080/api/Privs/get/'+id,options);
  let resultat = await response.json();
  alert("le nom de ce privilege est : "+resultat.nom_privs+" \n description : "+resultat.description);
}
supprimer2= async(id2)=>{

  fetch('http://localhost:8080/api/ProfilPrivs/supprimer/'+this.state.idshowprofil+"/"+id2,{
    method: 'POST',
    headers: new Headers({
        'Content-Type': 'application/json','Authorization': 'bearer '+sessionStorage.getItem('token')
    })
 ...
 let options = {method:"POST",headers : { "charset":"utf-8",'Content-Type':'application/json','Authorization': 'bearer '+sessionStorage.getItem('token') }};
  let response = await fetch("http://localhost:8080/api/profil/get/"+this.state.idshowprofil,options);
  let resultat = await response.json();
