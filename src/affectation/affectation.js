...
>
  ...
  download=async()=>{
    ...
    let response = await fetch('http://localhost:8080/api/Privs/list',options);
...
ajouter=e=>{
  ...
  if(this.state.profil!==""&& this.state.privs!==""){
        fetch("http://localhost:8080/api/ProfilPrivs/ajouter/"+this.state.profil+"/"+this.state.privs, {
          method: 'POST',
          headers: new Headers({
