...
>
  ...
  recherche=(e)=>{ 
    ...
    download=async()=>{
     ...
     let response = await fetch('http://localhost:8080/api/Chauffeur/list',options);
...
token:sessionStorage.getItem('token')
    ...
    //   fetch("http://localhost:8080/api/Chauffeur/ajouter", {
    //     method: 'POST',
    //     headers: new Headers({
...
supprimer=async(id)=>{
  ...
  fetch("http://localhost:8080/api/Chauffeur/supprimer/"+id, {
    method: 'POST',
    headers: new Headers({
        'Content-Type': 'application/json','Authorization': 'bearer '+sessionStorage.getItem('token')
    })
